---
title: "Using data to see if my boomer dad was right about today’s music"
...
```
## Reading the data set and loading the necessary libraries
...
head(spotify %>% 
  ...
  arrange(desc(count)), 100) # There's at least 100 duplicated tracks.... this will be annoying 
...
```{r}
not_one_hit_wonder = spotify_updated %>% 
  group_by(main.artist) %>% 
  dplyr::summarise(count = n()) %>% 
  arrange(desc(count)) %>% 
  filter(count > 2) # 8353 artists with at least 3 songs on Spotify 

non_one_hit_wonder_artist = as.character(not_one_hit_wonder$main.artist)

paste(non_one_hit_wonder_artist[1:10], collapse = "$|^")
paste(non_one_hit_wonder_artist[11:20], collapse = "$|^")
paste(non_one_hit_wonder_artist[21:30], collapse = "$|^")

# Will use this process to get a few hundred names at a time and use the REGEX expression "\\" to allow for the capture of the unintended symbols when using str_detect. Perform this over and over until all of the names are captured.  
...
## Analyse audio features based on year of release
...
spotify_updated %>% 
  ...
  labs(x = "Release Year", y = "Number of Tracks on Spotify") +
  scale_x_continuous(breaks = seq(from = 1920, to = 2020, by = 10))
...
spotify_updated %>% 
  ...
  labs(x = "Year", y = "Length of Track (in Seconds)") + 
  scale_x_continuous(breaks = seq(from = 1920, to = 2020, by = 10)) + 
  ylim(0, 300)
```
...
spotify_updated %>% 
  ...
  labs(x = "Year", y = "Tempo of Track (in BPM)") + 
  scale_x_continuous(breaks = seq(from = 1920, to = 2020, by = 10)) + 
  ylim(0, 150)
```
...
spotify_updated %>% 
  ...
  labs(x = "Year", y = "Mean Loudness Scores (in dB)") + 
  scale_x_continuous(breaks = seq(from = 1920, to = 2020, by = 10)) 
...
spotify_updated %>% 
  ...
  labs(x = "Year", y = "Popularity Scores", caption = "FIGURE8. Average Popularity of Track on Spotify According to Year of Release") + 
  scale_x_continuous(breaks = seq(from = 1920, to = 2020, by = 10)) + 
...
## Making a data set comparing popular from the 70s + 80s to those in the past 10 years 
...
spotify_comparison_longer_decade_group = spotify_comparison %>% 
  pivot_longer(
    cols = c(acousticness, danceability, energy, liveness, speechiness, valence, instrumentalness), 
    names_to = "features", 
...
## Examine the comparison based on group here 

```{r}
spotify_comparison_longer_decade_group %>% 
  ...
  scale_fill_manual("legend", values = c("current" = "darkgoldenrod", "classics" = "slateblue")) + 
  ...
  labs(x = "Comparison Groups", y = "Mean Feature Scores") + 
  guides(fill = F)
...
summarisation %>%
  kbl(longtable = T,  caption = "TABLE1. Comparison of Audio & Track Features in Songs between Past and Current Top Artists") %>%
...
```{r}
min(spotify_updated$year[spotify_updated$main.artist == "Maluma"])
min(spotify_updated$year[spotify_updated$main.artist == "Powfu"])
min(spotify_updated$year[spotify_updated$main.artist == "Wisin"])
min(spotify_updated$year[spotify_updated$main.artist == "Jay Wheeler"])
min(spotify_updated$year[spotify_updated$main.artist == "24kGoldn"])
min(spotify_updated$year[spotify_updated$main.artist == "Cardi B"])

min(spotify_updated$year[spotify_updated$main.artist == "BLACKPINK"])
min(spotify_updated$year[spotify_updated$main.artist == "Calvin Harris"])
min(spotify_updated$year[spotify_updated$main.artist == "Dermot Kennedy"])
min(spotify_updated$year[spotify_updated$main.artist == "SZA"])
min(spotify_updated$year[spotify_updated$main.artist == "TWICE"])
min(spotify_updated$year[spotify_updated$main.artist == "Why Dont We"])
min(spotify_updated$year[spotify_updated$main.artist == "YNW Melly"])
min(spotify_updated$year[spotify_updated$main.artist == "Miley Cyrus"])
min(spotify_updated$year[spotify_updated$main.artist == "Bebe Rexha"])
min(spotify_updated$year[spotify_updated$main.artist == "Black Eyed Peas"])
min(spotify_updated$year[spotify_updated$main.artist == "Little Mix"])
min(spotify_updated$year[spotify_updated$main.artist == "ZAYN"])
min(spotify_updated$year[spotify_updated$main.artist == "Ava Max"])
min(spotify_updated$year[spotify_updated$main.artist == "Danna Paola"])
min(spotify_updated$year[spotify_updated$main.artist == "David Guetta"])
min(spotify_updated$year[spotify_updated$main.artist == "Regard"])
min(spotify_updated$year[spotify_updated$main.artist == "Jack Harlow"])
min(spotify_updated$year[spotify_updated$main.artist == "Dalex"])
min(spotify_updated$year[spotify_updated$main.artist == "MAX"])
min(spotify_updated$year[spotify_updated$main.artist == "Camilo"])
